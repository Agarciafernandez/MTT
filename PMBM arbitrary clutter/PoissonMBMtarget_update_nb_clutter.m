function filter_upd=PoissonMBMtarget_update_nb_clutter(filter_pred,z,H,R,p_d,k,gating_threshold,Nhyp_max,log_pdf_clutter)

%PMBM update with Gibbs sampling for negative binomial clutter
...
for m=1:size(z,2)
    ...
    if(~isempty(indices_new_tracks))
        ...
        %With arbitrary clutter eB=1 as the following line is not required
        ...
        %clutter)
        weightB=0; %For PPP clutter, here we would have weightB=intensity_clutter;
...
if(Nprev_tracks==0) 
    ...
    cost_matrix_log=-Inf(Nnew_tracks,size(z,2));
    ...
    %Gibbs sampling to select k-good global hypotheses
    [opt_indices,nlcost]=Gibbs2DAssign_nb(-cost_matrix_log',kbest,log_pdf_clutter);

    opt_indices=opt_indices';
    nlcost=nlcost';

    globHyp=zeros(size(opt_indices));

    for i=1:Nnew_tracks

        for j=1:size(opt_indices,1)
            index_track=find(opt_indices(j,:)==i);
            if(isempty(index_track))
                index=0;
            else
                index=1;
            end
            globHyp(j,i)=index; %It is either 0 or 1
        end
    end
    ...
    for p=1:length(filter_pred.globHypWeight)
        ...
        %Hack so that we do not remove columns or rows (required for NB
        %clutter)
        indices_stay_row=1:size(cost_matrix_log,1);

        indices_stay_column=1:size(cost_matrix_log,2);
        ...
        %Hack so that we do not remove columns or rows (required for NB
        %clutter)
        fixed_indices_stay_column=[];
        fixed_indices_stay_row=[];
        ...
        if(isempty(cost_matrix_log_trimmed))
            ...
            %Gibbs sampling to select k-good global hypotheses
            [opt_indices,nlcost]=Gibbs2DAssign_nb(-cost_matrix_log_trimmed',kbest,log_pdf_clutter);
            ...
            for i=1:size(opt_indices,1)
                indices_higher0=opt_indices(i,:)>0;
                opt_indices_higher0=opt_indices(i,indices_higher0);

                opt_indices_trans(i,indices_stay_column(indices_higher0))=indices_stay_row(opt_indices_higher0);
