function filter_upd=TPMBM_all_update_range_bearing(filter_pred,z,p_d,k,gating_threshold,intensity_clutter,Nhyp_max,Lscan,T_alive,...
...
%TPMBM update for all trajectories with range-bearings measurements 
...
for i=1:Nprev_tracks
    ...
    for j=1:Nhyp_i %We go through all hypotheses
        ...
        if(prob_alive_j>T_alive)
            ...
            cross_p_d_mean=p_d_mean*prob_alive_j;
            ...
            filter_upd.tracks{i}.eB(j)=eB_j*(1-cross_p_d_mean)/(1-eB_j+eB_j*(1-cross_p_d_mean));
            filter_upd.tracks{i}.aHis{j}=[aHis_j,0];
            filter_upd.tracks{i}.weightBLog_k(j)=log(1-eB_j+eB_j*(1-cross_p_d_mean)); %Weight only at time step k (removing previous weight)
            
            %Update the length probabilities
            prob_length_j_upd=prob_length_j;
            prob_length_j_upd(1)=prob_length_j_upd(1)*(1-p_d_mean);
            prob_length_j_upd=prob_length_j_upd/(1-cross_p_d_mean);
            ...
            for m=1:size(z,2)
                ...
                if(maha<gating_threshold)
                    ...
                    filter_upd.tracks{i}.weightBLog_k(index_hyp)=log(eB_j*likelihood_i*prob_alive_j) ; %We multiply by prob_alive_j
