...
mesurement_dir_flag=1; %Measurement directed linearisation flag
likelihood_corr_flag=1; %Flag for perfoming likelihood correction. If set to zero, we do the Gaussian approximation to the likelihood with constant pd (taken at the prior mean)
%We also take the value at the mean of pd for misdetection hypotheses (and
%PPP)
...
save(['PMB_rb_kappa',int2str(kappa),'_R_range',int2str(R_range),'_clut',int2str(l_clutter),'_m_dir_flag', int2str(mesurement_dir_flag),'_l_corr_flag',int2str(likelihood_corr_flag), '_Nit_iplf', int2str(Nit_iplf),'_thre_iplf',int2str(10000*threshold_iplf)]);
