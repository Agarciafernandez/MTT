%Demo with the implementation of the trajectory multi-Bernoulli mixture (TMB)
...
%"Trajectory Poisson multi-Bernoulli filters", accepted in IEEE
%Transactions on Signal Processing

%Filter for all trajectories
...
existence_estimation_threshold1=0.5; %Only for estimator 1
...
for i=1:Nmc
    ...
    for k=1:Nsteps
        ...
        filter_upd=TMBM_all_update(filter_pred,z,H,R,p_d,k,gating_threshold,intensity_clutter,Nhyp_max,Lscan,T_alive);
          ...
          %TMB projection
        filter_upd_pmb=TMB_all_projection(filter_upd,T_alive,Nx,k,Lscan);
...
%save(['TMB_all_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Nhyp_max',int2str(Nhyp_max),'_Lscan',int2str(Lscan)])
