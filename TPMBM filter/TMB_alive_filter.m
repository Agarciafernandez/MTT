%Demo with the implementation of the trajectory multi-Bernoulli (TMB)
%filter described in
...
for i=1:Nmc
    ...
    for k=1:Nsteps
        ...
        %TMB projection
        filter_upd_pmb=TMB_projection(filter_upd);
...
%save(['TMB_alive_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Nhyp_max',int2str(Nhyp_max),'_Lscan',int2str(Lscan)])
