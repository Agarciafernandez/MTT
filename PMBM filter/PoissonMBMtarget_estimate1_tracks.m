...
for i=1:size(tag_estimate_targets,2)
    ...
    if(isempty(index_traj))
        %We have a new trajectory that we start at the time of the first target
        %estimate
        N_prev_trajectories=N_prev_trajectories+1;
        t_b_estimate=[t_b_estimate,k];
        length_estimate=[length_estimate,1];
        tag_estimate=[tag_estimate,tag_estimate_targets_i];
        
        X_estimate{N_prev_trajectories}=X_estimate_targets_i;
        
    else
        %We append the estimated target state to a previous trajectory, filling holes with NaN
        X_estimate_current=X_estimate{index_traj};
        length_estimate(index_traj)=k-t_b_estimate(index_traj)+1;
        
        
        X_estimate_traj=nan(Nx*length_estimate(index_traj),1);
