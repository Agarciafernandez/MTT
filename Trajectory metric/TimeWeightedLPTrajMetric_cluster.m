function [dxy, loc_cost, miss_cost, fa_cost, switch_cost]=TimeWeightedLPTrajMetric_cluster(X, Y, c, p, gamma,time_weights1,time_weights2)
% This function computes the time weighted LP metric between sets of trajectories in
...
% The non time-weighted metric is proposed in
...
% time_weights1 is the time weight vector Tx1 for the
% localisation/missed/false target costs
% time_weights2 is the time weight vector (T-1)x1 for the
% switching costs
% Note that the time weights must be column vectors
% -------------------------------------------------------------------------
% Output:
% dxy: Metric value
...
% -------------------------------------------------------------------------
% Authors: Ángel F. García-Fernández and Abu S. Rahmatullah
% This code uses clustering, see Sec. IV.D in [2], to compute the solution.
...
if(or(length(time_weights1)~=T, length(time_weights2)~=T-1))
   error('Time weights in time weighted LP trajectory metric do not have the correct dimensions')
end
...
for i=1:length(Clusters)
    ...
    if(length(Cluster_i)==1)
        ...
        if(isempty(i_x))
            ...
            dxy_i=c^p/2*sum(time_weights1(isrealY_i));
            ...
            dxy_i=c^p/2*sum(time_weights1(isrealX_i));
    ...
    elseif(length(Cluster_i)==2)
        ...
        loc_cost(index4)=loc_cost(index4)+DAB_i(index4).*time_weights1(index4);
        ...
        for j=1:length(X_i.tVec)
            ...
            miss_cost_i(t_axis)=miss_cost_i(t_axis)+c^p/2*squeeze(isrealX_i(1,t_axis,j))'.*time_weights1(isrealX_i(1,t_axis,j));
        end
        
        for j=1:length(Y_i.tVec)
            t_axis=[Y_i.tVec(j):t_min-1, t_max+1:tf_Y(j)];
            fa_cost_i(t_axis)=fa_cost_i(t_axis)+c^p/2*squeeze(isrealY_i(1,t_axis,j))'.*time_weights1(isrealY_i(1,t_axis,j));
        end
        ...
        [dxy_i,loc_cost_i2, miss_cost_i2, fa_cost_i2, switch_cost_i2]=TimeWeightedLP_metric_cluster(X_i,Y_i,DAB_i,...
            nx_i,ny_i,nxny_i,nxny2_i,T_i,c,p,gamma,time_weights1(t_min:t_max),time_weights2(t_min:t_max-1));
...
function [dxy,loc_cost, miss_cost, fa_cost, switch_cost]=TimeWeightedLP_metric_cluster(X,Y,DAB,nx,ny,nxny,nxny2,T,c,p,gamma,time_weights1,time_weights2)
...
%Time weighted DAB
time_weights1_tensor=repmat(time_weights1',size(DAB,1),1,size(DAB,2));
time_weights1_tensor=permute(time_weights1_tensor,[1 3 2]);
DAB_TW=DAB.*time_weights1_tensor;

f(WtPos) = reshape(DAB_TW, [WtLen,1]); % for vec(W(1)) to vec(W(T)), loc cost
f(etPos) = 0.5 * gamma^p * time_weights2; %for e(1) to e(T-1), switch cost
...
loc_cost=loc_cost.*time_weights1;
miss_cost=miss_cost.*time_weights1;
fa_cost=fa_cost.*time_weights1;
switch_cost=switch_cost.*time_weights2;
