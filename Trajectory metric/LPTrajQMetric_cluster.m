function [dxy, loc_cost, miss_cost, fa_cost, switch_cost]=LPTrajQMetric_cluster(X, Y, c, p, gamma,rho)
% This function computes the LP GOSPA quasimetric between sets of trajectories defined
% in A. F. García-Fernández, J. Gu, L. Svensson, Y. Xia, J. Krejcí, O. Kost, O. Straka “GOSPA and T-GOSPA quasi-metrics for evaluation of multi-object tracking algorithms,” 
% accepted in IEEE Transactions on Aerospace and Electronic Systems, 2026.
% The q-metric can penalise missed and false targets with different costs
...
% rho: 0<rho<1, coefficient for penalty of false targets 
% -------------------------------------------------------------------------
% Output:
% dxy: Quasi-Metric value
...
%Cost for a single missed and a false target
cost_missed_single = (1-rho)*c^p;
cost_false_single = rho*c^p;

%%%%%%%%%% localisation cost computation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
DAB = locCostComp_v2(X, Y, c, p,cost_missed_single,cost_false_single);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:length(Clusters)
    ...
    if(length(Cluster_i)==1)
        ...
        if(isempty(i_x))
            ...
            dxy_i=cost_false_single*sum(isrealY_i);
            fa_cost(t_axis)=fa_cost(t_axis)+cost_false_single*isrealY_i;
        else
            %Then it is a trajectory in X
            list_x=Cluster_i(i_x);
            t_axis=X.tVec(list_x):X.tVec(list_x)+ X.iVec(list_x)-1;
            isrealX_i=~isnan(X.xState(1,t_axis,list_x))';
            dxy_i=cost_missed_single*sum(isrealX_i);
            miss_cost(t_axis)=miss_cost(t_axis)+cost_missed_single*isrealX_i;
        end
    elseif(length(Cluster_i)==2)
        ...
        index1=DAB_i==c^p;
        fa_cost(index1)=fa_cost(index1)+cost_false_single;
        miss_cost(index1)=miss_cost(index1)+cost_missed_single;
        
        %False targets (when X does not exist)
        index2=and(DAB_i==cost_false_single,or(isnan_X,no_exist_X));
        fa_cost(index2)=fa_cost(index2)+cost_false_single;
        

        %Missed targets (when Y does not exist)
        index3=and(DAB_i==cost_missed_single,or(isnan_Y,no_exist_Y));
        miss_cost(index3)=miss_cost(index3)+cost_missed_single;
        ...
        for j=1:length(X_i.tVec)
            ...
            miss_cost_i(t_axis)=miss_cost_i(t_axis)+cost_missed_single*squeeze(isrealX_i(1,t_axis,j))';
        end
        
        for j=1:length(Y_i.tVec)
            t_axis=[Y_i.tVec(j):t_min-1, t_max+1:tf_Y(j)];
            fa_cost_i(t_axis)=fa_cost_i(t_axis)+cost_false_single*squeeze(isrealY_i(1,t_axis,j))';
        end
        ...
        [dxy_i,loc_cost_i2, miss_cost_i2, fa_cost_i2, switch_cost_i2]=LP_metric_cluster(X_i,Y_i,DAB_i,nx_i,ny_i,nxny_i,nxny2_i,T_i,c,p,gamma,cost_missed_single,cost_false_single);
...
function [dxy,loc_cost, miss_cost, fa_cost, switch_cost]=LP_metric_cluster(X,Y,DAB,nx,ny,nxny,nxny2,T,c,p,gamma,cost_missed_single,cost_false_single)
...
[loc_cost, miss_cost, fa_cost, switch_cost] ...
    = computeLocFalseMissedSwitchCosts(wMat, DAB, X, Y, c, p, gamma,cost_missed_single,cost_false_single);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end

function locCostMat = locCostComp_v2(X, Y, c, p,cost_missed_single,cost_false_single)
            ...
            if (t>=X.tVec(xind)) && (t<=(X.tVec(xind)+X.iVec(xind)-1)) && ~isnan(X.xState(1,t,xind))
                ...
                for yind = 1:ny+1
                    if (yind <= ny) && (t >= Y.tVec(yind)) && ...
                        ...
                        locCostMat(xind,yind,t) = computeLocCostPerTime( ...
                            X.xState(:,t,xind), Y.xState(:,t,yind), c, p,cost_missed_single,cost_false_single);
                        
                    else % yind does not exist or yind is dummy
                        locCostMat(xind,yind,t) = cost_missed_single;
                    end
                end
            else        
                ...
                for yind = 1:ny
                    if (t >= Y.tVec(yind)) && t <= Y.iVec(yind) + Y.tVec(yind) - 1 &&  ~isnan(Y.xState(1,t,yind))
                        % if Y_yind exists at t
                        locCostMat(xind,yind,t) = cost_false_single;
                    end
                end
            end
        else    % xind is dummy
...
function d = computeLocCostPerTime(x, y, c, p,cost_missed_single,cost_false_single)
if all(~isnan(x)) && all(~isnan(y))
    % neither x nor y has hole
    d = min(norm(x-y, p)^p,c^p);
elseif any(isnan(x) & ~isnan(y)) 
    % x has a hole, y does not has a hole
    d = cost_false_single;
elseif any(~isnan(x) & isnan(y))
    % y has a hole, x does not have a hole
    d = cost_missed_single;
else
    d = 0;
end
end
...
function [loc_cost, miss_cost, fa_cost, switch_cost] ...
    = computeLocFalseMissedSwitchCosts(w_mat, locCostMat, X, Y, c, p, gamma,cost_missed_single,cost_false_single)
...
miss_cost = cost_missed_single * squeeze(sum(sum(w_mat .* miss_mask, 1), 2))+ cost_missed_single * squeeze(sum(sum(w_mat .* fa_miss_mask, 1), 2));
fa_cost = cost_false_single * squeeze(sum(sum(w_mat .* fa_mask, 1), 2))+ cost_false_single * squeeze(sum(sum(w_mat .* fa_miss_mask, 1), 2));
end
