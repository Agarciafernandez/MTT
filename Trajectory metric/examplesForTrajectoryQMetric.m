%This file provides a simple demo with one-dimensional target states to use the TGOSPA quasi-metric on sets of trajectories
...
example_id=2; %Value from 1 to 7

% parameters of the quasi-metric
c=5; 
p=1; 
gamma=1;
rho=0.7; %rho must be between 0 and 1
...
[dxy_metric, loc_cost_metric, miss_cost_metric, fa_cost_metric, switch_cost_metric]=LPTrajMetric_cluster(X, Y, c, p, gamma)
[dxy, loc_cost, miss_cost, fa_cost, switch_cost]=LPTrajQMetric_cluster(X, Y, c, p, gamma,rho)

%Note loc_cost,miss_cost, fa_cost and switch_cost are already given to the p-th
%power
disp('Metric value')
disp(dxy)
disp('T-GOSPA quasi-metric decomposition: localisation error, missed target cost, false target cost, switching cost')
...
ylabel('T-GOSPA quasi-metric decomposition')
xlabel('Time step')
