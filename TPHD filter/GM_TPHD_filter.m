%Implementation of the Gaussian mixture Trajectory PHD filter
...
%Filter parameters
Ncom_max=30;   %Maximum number of PHD component
T_pruning=10^(-4); %Pruning threshold for PHD
T_absorption=4; %Threshold for absorption
...
%Squared GOSPA errors and decomposition (alpha=2)
...
%Squared LP trajectory metric errors
...
for i=1:Nmc
    ...
    for k=1:Nsteps       
        ...
        %Update
        [weights_u, means_u,covs_u,t_ini_u,length_u,logical_actives_u,means_k_old_u]=GMTPHD_Lscan_filter_update(weights_k,...
            means_k,covs_k,t_ini_k,length_k,Ncom_k,logical_actives_k,z,H,R,p_d,l_clutter,Area,Lscan,means_k_old);
        ...
        %Estimation
        [X_estimate,t_b_estimate,length_estimate]=GMTPHD_estimation(weights_u, means_u,t_ini_u,length_u,Lscan,means_k_old_u);
        ...
        %Prediction (without new born trajectories)
        if(k<Nsteps)
...
%save(['TPHD_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Lscan',int2str(Lscan)])
...
rms_gospa_false_tot=sqrt(sum(sum(squared_gospa_fal_t_tot))/(Nmc*Nsteps))
