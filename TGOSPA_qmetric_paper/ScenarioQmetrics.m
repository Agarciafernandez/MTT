...
Q = 0.4*kron(eye(2),[T^3/3 T^2/2; T^2/2 T]);
H = kron(eye(2),[1 0]);
R = 4* eye(2);
...
p_s=0.99;
Area=[800 800];   
Nsteps=101; %Considered number of time steps in the simulation

l_clutter=15; %Mean number of clutter measurements (Poisson cardinality and uniformly distributed in the surveillance area)
...
rand('seed',5)
randn('seed',5)
[X_truth,t_birth,t_death]=TrajectoryWilliams15_qmetric(Scenario_number,Nsteps,F,numtruth,Q,Area);


% display('Only one target')
% X_truth=X_truth(1:4,:);
% t_birth=t_birth(1);
% t_death=t_death(1);
...
% display ('Three targets')
% X_truth=X_truth(1:12,:);
% t_birth=t_birth(1:3);
% t_death=t_death(1:3);

%display('Test birth')
%t_death(1)=20; %Para que no muera cuando estan juntos

%We only keep the second and third target
% X_truth=X_truth(5:12,:);
% t_birth=t_birth(2:3);
% t_death=t_death(2:3);
...
rho_qmetric=0.7; %Parameter rho of the GOSPA q metrics.
