%Demo with the implementation of the (track-oriented) Poisson multi-Bernoulli (PMB)
%filter described in
%J. L. Williams, "Marginal multi-bernoulli filters: RFS derivation of MHT, JIPDA, and association-based member," in IEEE Transactions on Aerospace and Electronic Systems, vol. 51, no. 3, pp. 1664-1687, July 2015, doi: 10.1109/TAES.2015.130550. 
...
% This code is the one for paper [1] where erformance is assessed via the T-GOSPA quasi-metric (which can penalised
%missed and false targets with different costs) and the T-GOSPA similarity
%score

%[1] A. F. García-Fernández, J. Gu, L. Svensson, Y. Xia, J. Krejcí, O. Kost, O. Straka “GOSPA and T-GOSPA quasi-metrics for evaluation of multi-object tracking algorithms,” 
...
%T-GOSPA_metric errors for the estimator
...
%T-GOSPA quasi-metric errors for the estimator
squared_LP_qmetric_t_tot=zeros(1,Nsteps);
squared_LP_qmetric_loc_t_tot=zeros(1,Nsteps); %Localisation error
squared_LP_qmetric_fal_t_tot=zeros(1,Nsteps); %False target error
squared_LP_qmetric_mis_t_tot=zeros(1,Nsteps); %Misdetection error
squared_LP_qmetric_switch_t_tot=zeros(1,Nsteps); %Track switching error

%T-GOSPA quasi-metric score
beta_score=CalibrateBetaScore(c_gospa,2,0.5,50,0.1);

score_TGOSPAq_t_tot=zeros(1,Nsteps);
score_TGOSPA_t_tot=zeros(1,Nsteps);

%We consider two options to evaluate T-GOSPA q-metric error.
...
for i=1:Nmc
    ...
    for k=1:Nsteps
        ...
        if(error_online==1)
                        %Computation of the squared T-GOSPA metric error
            [squared_LP_metric, LP_metric_loc, LP_metric_miss, LP_metric_fal, LP_metric_switch,score_TGOSPA]=ComputeLP_metric_all_error(X_estimate,t_b_estimate, length_estimate,X_truth,t_birth,t_death,c_gospa,gamma_track_metric,k,Nx,beta_score);
            ...
            score_TGOSPA_t_tot(k)=score_TGOSPA_t_tot(k)+score_TGOSPA;



            %Computation of the squared T-GOSPA quasi-metric error
            [squared_LP_qmetric, LP_qmetric_loc, LP_qmetric_miss, LP_qmetric_fal, LP_qmetric_switch,score_TGOSPAq]=ComputeLP_qmetric_all_error(X_estimate,t_b_estimate, length_estimate,X_truth,t_birth,t_death,c_gospa,gamma_track_metric,rho_qmetric,k,Nx,beta_score);
            squared_LP_qmetric_t_tot(k)=squared_LP_qmetric_t_tot(k)+squared_LP_qmetric;
            squared_LP_qmetric_loc_t_tot(k)=squared_LP_qmetric_loc_t_tot(k)+LP_qmetric_loc;
            squared_LP_qmetric_fal_t_tot(k)=squared_LP_qmetric_fal_t_tot(k)+LP_qmetric_fal;
            squared_LP_qmetric_mis_t_tot(k)=squared_LP_qmetric_mis_t_tot(k)+LP_qmetric_miss;
            squared_LP_qmetric_switch_t_tot(k)=squared_LP_qmetric_switch_t_tot(k)+LP_qmetric_switch;


            score_TGOSPAq_t_tot(k)=score_TGOSPAq_t_tot(k)+score_TGOSPAq;
            ...
            if(k==Nsteps)
                ...
                %Computation of the squared T-GOSPA quasi-metric error
                [squared_LP_qmetric, LP_qmetric_loc, LP_qmetric_miss, LP_qmetric_fal, LP_qmetric_switch]=ComputeLP_qmetric_all_error_final(X_estimate,t_b_estimate, length_estimate,X_truth,t_birth,t_death,c_gospa,gamma_track_metric,rho_qmetric,k,Nx);

                squared_LP_qmetric_t_tot=squared_LP_qmetric_t_tot+[zeros(1,Nsteps-1),squared_LP_qmetric];
                squared_LP_qmetric_loc_t_tot=squared_LP_qmetric_loc_t_tot+LP_qmetric_loc';
                squared_LP_qmetric_fal_t_tot=squared_LP_qmetric_fal_t_tot+LP_qmetric_fal';
                squared_LP_qmetric_mis_t_tot=squared_LP_qmetric_mis_t_tot+LP_qmetric_miss';
                squared_LP_qmetric_switch_t_tot=squared_LP_qmetric_switch_t_tot+[LP_qmetric_switch',0];
...
save(['PMB_qmetric_all_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Nhyp_max',int2str(Nhyp_max),'_rho',int2str(10*rho_qmetric)])
...
if(plot_figures)
    %Q-metrics plots
    figure(1)
    plot(1:Nsteps,sqrt(squared_LP_qmetric_t_tot/Nmc), 'b',...
        1:Nsteps,sqrt(squared_LP_metric_t_tot/Nmc), 'r','Linewidth',1.3)
    xlabel('Time step')
    ylabel('RMS  error')
    grid on
    legend('T-GOSPA q-metric','T-GOSPA metric')

    figure(2)
    plot(1:Nsteps,sqrt(squared_LP_qmetric_loc_t_tot/Nmc),  'b',...
        1:Nsteps,sqrt(squared_LP_metric_loc_t_tot/Nmc),  'r','Linewidth',1.3)
    xlabel('Time step')
    ylabel('RMS  localisation error')
    grid on


    figure(3)
    plot(1:Nsteps,sqrt(squared_LP_qmetric_mis_t_tot/Nmc), 'b',...
        1:Nsteps,sqrt(squared_LP_metric_mis_t_tot/Nmc),  'r','Linewidth',1.3)
    xlabel('Time step')
    ylabel('RMS missed target cost')
    grid on

    figure(4)
    plot(1:Nsteps,sqrt(squared_LP_qmetric_fal_t_tot/Nmc),'b',...
        1:Nsteps,sqrt(squared_LP_metric_fal_t_tot/Nmc),'r','Linewidth',1.3)
    xlabel('Time step')
    ylabel('RMS false target cost')
    grid on

    figure(5)
    plot(1:Nsteps,sqrt(squared_LP_qmetric_switch_t_tot/Nmc),'b', ...
        1:Nsteps,sqrt(squared_LP_metric_switch_t_tot/Nmc),'r','Linewidth',1.3)
    xlabel('Time step')
    ylabel('RMS track switching cost')
    grid on

end

if(error_online)
    display('T-GOSPA quasi-metric errors')
    sqrt(sum(squared_LP_qmetric_t_tot)/(Nmc*Nsteps))
    sqrt(sum(squared_LP_qmetric_loc_t_tot)/(Nmc*Nsteps))
    sqrt(sum(squared_LP_qmetric_fal_t_tot)/(Nmc*Nsteps))
    sqrt(sum(squared_LP_qmetric_mis_t_tot)/(Nmc*Nsteps))
    sqrt(sum(squared_LP_qmetric_switch_t_tot)/(Nmc*Nsteps))
    ...
    sqrt(sum(squared_gospa_mis_t_tot)/(Nmc*Nsteps))
        ...
        display('Average T-GOSPA quasi-metric score')
    sum(score_TGOSPAq_t_tot)/(Nmc*Nsteps)
    
    display('Average T-GOSPA metric score')
    sum(score_TGOSPA_t_tot)/(Nmc*Nsteps)
