%Continous-discrete trajectory Poisson multi-Bernoulli (CD-TPMB)
...
%hypotheses that the trajectory died at a previous time step. For example, mean_past{1}{1} (stores mean at k-1 
%for hypothesis 1,) , mean_past{1}{2} (stores mean at k-1 for hypothesis 2).... that stores the
%previous means
...
%Code is similar to CD-TPMBM code but making TPMB projection after each update
...
process_oos_flag=1; %If we set it to zero, we discard oos measurements. It provides a baseline for comparison.
...
existence_estimation_threshold1=0.5; %Only for esimator 1
...
for i=1:Nmc
    ...
    for k=1:Nsteps
        ...
        if(is_k_indices(k))
            ...
            %TPMB projection
            filter_upd=TPMB_all_projection_cov(filter_upd,T_alive,Nx,k_is,Lscan);
        ...
        elseif(process_oos_flag)
            ...
            if(ko>=k_is-Lscan+2)
                ...
                filter_upd=TPMB_all_projection_cov_oos_round(filter_upd,T_alive,Nx,k_is,Lscan);
...
save(['TPMB_oos_all_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Nhyp_max',int2str(Nhyp_max),'_Lscan',int2str(Lscan),'_oos',int2str(process_oos_flag),'_lambdaoos',int2str(lambda_oos)])
