...
for i=1:Nmc
    ...
    for k=1:Nsteps
        ...
        if(k<Nsteps)
            ...
            %Prediction (survival targets) PHD
            [weights_k, means_k,covs_k,Ncom_k,logical_active_k]=GMPHD_filter_prediction_offset(weights_u, means_u,covs_u,Ncom_u,logical_active_u,F_k,Q_k,p_s_k,b_k);


            %Prediction (we add the new born targets) PHD
...
save(['CPHD_cd_linear_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_lambda',int2str(100*lambda),'_mu',int2str(100*mu)])
