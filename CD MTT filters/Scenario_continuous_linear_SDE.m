...
%SDE parameters (We use the Ornstein-Uhlenbeck process for the velocity in
% L. M. Millefiori, P. Braca, K. Bryan and P. Willett, "Modeling vessel kinematics using a stochastic mean-reverting process for long-term prediction," 
% in IEEE Transactions on Aerospace and Electronic Systems, vol. 52, no. 5, pp. 2313-2330, October 2016,
gamma=0.1;
...
u=[0,gamma*velocity(1),0,gamma*velocity(2)]';
...
for i=1:N_targets_tot
    ...
    [F_k,b_k,Q_k]=CalculateTransition_linear_SDE(A,u,time_lag_i,Q_c);
    
    

    %Sample of target state at time of birth 
    X_b=F_k*X_a+b_k+chol(Q_k)'*randn(Nx,1);
    ...
    for k=time_aux+1:t_death(i)-1
        ...
        [F_k,b_k,Q_k]=CalculateTransition_linear_SDE(A,u,delta_tk_i,Q_c);
        
        mean_pred=F_k*X_truth(4*i-3:4*i,k-1)+b_k;
      

        X_truth(4*i-3:4*i,k)=mean_pred+chol(Q_k)'*randn(Nx,1);
    end
...
% 
%     text(X_truth(4*i-3,t_birth(i))-20,X_truth(4*i-1,t_birth(i))+0,num2str(t_birth(i)))
% end
% hold off
% xlabel('x position (m)')
% ylabel('y position (m)')
