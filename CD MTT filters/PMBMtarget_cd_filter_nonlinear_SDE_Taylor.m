%Implementation of the Continuous-discrete Poisson multi-Bernoulli mixture
...
for i=1:Nmc
    ...
    for k=1:Nsteps
        ...
        %Draw filter output
        %        DrawFilterEstimates(X_truth,t_birth,t_death,X_estimate,[0,Area(1)],[6350,Area(2)],z,k)
        %         pause
        ...
        if(k<Nsteps)
            ...
            %Prediction
            filter_pred=PoissonMBMtarget_pred_nonlinear_SDE_Taylor(filter_upd,f_drift, F_drift,Q_c,p_s_k,lambda_new_born_t(k+1),mean_b_k,cov_b_k,delta_tk_t(k+1),Nx);
        end
    end
...
save(['PMBM_cd_nonlinear_Taylor_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Nhyp_max',int2str(Nhyp_max),'_lambda',int2str(100*lambda),'_mu',int2str(100*mu)])
