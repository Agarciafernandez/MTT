function filter_pred=PoissonMBMtarget_pred_offset(filter_upd,F,Q,p_s,weights_b,means_b,covs_b,offset_mean)
%Author:  Ángel F. García-Fernández

%This prediction step includes an offset for the mean 

%Prediction for Poisson component
filter_pred.weightPois=p_s*filter_upd.weightPois;
filter_pred.meanPois=F*filter_upd.meanPois+offset_mean;
...
if(Ntracks>0)
    for i=1:Ntracks
        ...
        for j=1:Nhyp_i %We go through all hypotheses
            ...
            filter_pred.tracks{i}.meanB{j}=F*filter_upd.tracks{i}.meanB{j}+offset_mean;
