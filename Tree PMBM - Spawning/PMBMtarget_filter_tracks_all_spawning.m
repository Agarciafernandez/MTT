%Demo with an implementation of the Poisson multi-Bernoulli mixture (PMBM)
%filter with spawning in

%Z. Su, H. Ji, and Y. Zhang, “A Poisson multi-Bernoulli mixture filter with
%spawning based on Kullback-Leibler divergence minimization,” Chinese
%Journal of Aeronautics, 2021.

% This code is similar to the one in 
...
% adding the spawned targets to the PPP at each prediction step by performing recycling
...
for i=1:Nmc
    ...
    for k=1:Nsteps
        ...
        %Prediction
        filter_pred=PMBMtarget_pred_spawning(filter_upd,F,Q,p_s,weights_b,means_b,covs_b,F1,F2,p_s1,p_s2,d_spawning);
...
save(['S-PMBM_tracks_all_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Nhyp_max',int2str(Nhyp_max)])
