function filter_upd=TrPMBM_all_update(filter_pred,z,H,R,p_d,k,gating_threshold,intensity_clutter,Nhyp_max,Lscan,T_alive)

%TrPMBM update for all tree trajectories
...
for m=1:size(z,2)
    ...
    if(~isempty(indices_new_tracks))
        ...
        %For spawning, we also have a genealogy variable (it is actually
        %the genealogy variable assuming the branch is alive)
        filter_upd.tracks{Nprev_tracks+m}.genealogy=ones(filter_pred.Pois{index_i_max}.length_Pois,1);
        ...
        filter_upd.tracks{Nprev_tracks+m}.prob_length{1}=1;
        filter_upd.tracks{Nprev_tracks+m}.mean_past{1}=cell(0,1);
        filter_upd.tracks{Nprev_tracks+m}.genealogy=[];
...
%We initialise the variable for tree indices
N_tree_prev=filter_pred.N_tree;
tree_index_prev=filter_pred.tree_index;

filter_upd.tree_index=[tree_index_prev;(N_tree_prev+1:N_tree_prev+Nnew_tracks)'];
filter_upd.N_tree=N_tree_prev+Nnew_tracks;
...
for i=1:Nprev_tracks
    ...
    %We copy the genealogy variable
    filter_upd.tracks{i}.genealogy=filter_pred.tracks{i}.genealogy;
