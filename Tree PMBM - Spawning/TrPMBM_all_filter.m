%Demo with the implementation of the Tree Poisson multi-Bernoulli
%mixture (TrPMBM) with tree trajectories for multiple target target with
...
for i=1:Nmc
    ...
    filter_pred.Pois{1}.weightPois=weights_b;
    ...
    %tracks/branches.
    filter_pred.tree_index=[];
    filter_pred.N_tree=0; %Number of potentially detected trees, i.e., number of received measurements up to now
    ...
    for k=1:Nsteps
        ...
        filter_upd=TrPMBM_all_update(filter_pred,z,H,R,p_d,k,gating_threshold,intensity_clutter,Nhyp_max,Lscan,T_alive);
...
%          DrawTreeFilterEstimates(X_truth,t_birth,t_death,father_spawning,X_estimate,t_b_estimate,tree_index_estimate,genealogy_estimate,[0,600],[0,400],z,k)
        ...
        filter_upd_pruned=TrPMBM_all_pruning(filter_upd, T_pruning,T_pruningPois,Nhyp_max,existence_threshold,T_alive);
        ...
        %Prediction
        filter_pred=TrPMBM_all_prediction(filter_upd,F,Q,p_s,weights_b,means_b,covs_b,Lscan,k,T_alive,F1,F2,p_s1,p_s2,d_spawning);
...
save(['TrPMBM_all_pd',int2str(100*p_d),'_R',int2str(R(1,1)),'_clut',int2str(l_clutter),'_Nhyp_max',int2str(Nhyp_max),'_Lscan',int2str(Lscan)])
