function filter_upd_pruned=PMBMtarget_pruning_merging(filter_upd, T_pruning,T_pruningPois,Nhyp_max,existence_threshold,T_mergingPois,NcomPois_max)
%This function performs pruning of Bernoulli components (using existence
%threshold), global hypotheses (T_pruning and Nhyp_max)
%and PPP components (T_pruningPois).
%It also performs merging of PPP components.
...
%Pruning and merging of PPP components using the function GMPHD_filter_pruning.m (for
%PHD filtering)
logical_active_u=true(1,length(filter_upd.weightPois));
[weights_o, means_o,covs_o,Ncom_o,logical_actives_o]=GMPHD_filter_pruning(filter_upd.weightPois, filter_upd.meanPois,filter_upd.covPois,logical_active_u,NcomPois_max,T_pruningPois,T_mergingPois);


filter_upd_pruned.weightPois=weights_o(logical_actives_o);
filter_upd_pruned.meanPois=means_o(:,logical_actives_o);
filter_upd_pruned.covPois=covs_o(:,:,logical_actives_o);
...
for i=1:Ntracks
    ...
    indices_valid=globHyp_pruned(:,i); %Track indices that appear in a global hypothesis
    %We remove single target hypothesis that do not appear in the global hypothesis
    index_remove=~ismember(1:Nhyp_i,indices_valid);
